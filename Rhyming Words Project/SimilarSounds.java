...
class SimilarSounds
{
	...
	public static void populateWordToSoundMap(List<String> lines) {	
		...
		for(String items : lines){
			key = Extractor.extractWordFromLine(items);
			value = Extractor.extractSoundFromLine(items);
			wordToSound.put(key, value);
		}
	}
	...
	public static void populateSoundGroupToSimilarWordsMap(List<String> lines) {
		...
		String value;
		for(String items : lines){
			String word = Extractor.extractWordFromLine(items);
			String sound = Extractor.extractSoundFromLine(items);
			...
			if(soundGroupToSimilarWords.get(key) == null){
				...
				newBST.insert(value);
				soundGroupToSimilarWords.put(key, newBST);
				continue;
			}
			soundGroupToSimilarWords.get(key).insert(value);
		}
	}
	...
	public static void findSimilarWordsInList(String words[]) {
		...
		for(int x = 0; x < words.length; x++){
			int foundMatches = 0;
			String word = words[x];
			String sound = wordToSound.get(word.toUpperCase());
			String soundGroup = "";
			try{
			...
			BST<String> similarWords = new BST<>();
			similarWords = soundGroupToSimilarWords.get(soundGroup);
			
			if(usedWords.contains(word)){
				continue;
			}
			else{
				System.out.print("\"" + word + "\" sounds similar to: ");
				usedWords.add(word);
			}

			for(int y = x; y < words.length; y++){
				String otherWord = words[y];
				if((similarWords.find(otherWord.toUpperCase()) != null) && usedWords.contains(otherWord) == false){
					System.out.print("\"" +  otherWord + "\" ");
					usedWords.add(otherWord);
					foundMatches++;
				}
				else if(y+1 == words.length && foundMatches == 0){
	...
	public static void findSimilarWordsTo(String theWord) {		
		...
		BST<String> similarWords = soundGroupToSimilarWords.get(soundGroup);
		System.out.print("Words similar to \"" + theWord + "\": " + similarWords);
	}
